...
if( isset( $_POST['submit'])) {
	...
	if( $newpassword1 == $newpassword2 ) {
		$stmnt = 'select * from school where  School_id = :schoolcode and password = :password';
		$params = array(
		':schoolcode' => $_SESSION['userid'],
		':password' => $cpassword
		);
		//$row =  $db->display($stmnt, $params);
		if( $db->display($stmnt, $params) ) {	
			$stmnt = 'update school set password = :newpassword1 where  School_id = :schoolcode';
			$params = array(
			':schoolcode' => $_SESSION['userid'],
